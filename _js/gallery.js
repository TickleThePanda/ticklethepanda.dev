(function() {
  ...
  function loadGalleries() {
    return fetch('https://api.ticklethepanda.co.uk/gallery')
            ...
            return gallery.images.map((image) => {
              return `
                <a class="${ image.favourite ? 'favourite' : '' }"
                    href="https://s.ticklethepanda.co.uk${fullPath}${gallery.reference}/${image.fileName}">
                  <img src="https://s.ticklethepanda.co.uk${ image.favourite ? fullPath : thumbsPath }${ gallery.reference }/${ image.fileName }">
                </a>`;
            }).join("");
          }
          return `
