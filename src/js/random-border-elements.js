window.addEventListener('load', function () {
  ...
  function generateRandomCardClipPath() {
    ...
    for (let randomBorderElement of randomBorderElements) {
      const originalClipPath = generateRandomCssPolygonForElement(randomBorderElement);
      ...
      if (randomBorderElement.classList.contains('js-random-border-hover')) {
        ...
        randomBorderElement.addEventListener('mouseenter', function() {
          ...
          const animationTime = parseInt(
            getComputedStyle(randomBorderElement)
              .getPropertyValue('--random-border--animation-duration-ms')
          ) || 0;

          const currentClipPath = getComputedStyle(randomBorderElement).getPropertyValue('clip-path');

          if (animation && animation.playState === 'running') {
            animation.cancel();
          }

          const newClipPath = generateRandomCssPolygonForElement(randomBorderElement);
